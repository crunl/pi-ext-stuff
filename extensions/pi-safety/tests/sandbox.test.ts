...
describe("sandbox integration", () => {
  it("resolves Guardian tilde paths against an explicit trusted home", async () => {
    const trustedHome = await mkdtemp(join(tmpdir(), "pi-safety-guardian-home-"));
  ...
  it("runs sandboxed Guardian read-only file operations without changing the parent manager", async () => {
    const directory = await mkdtemp(join(tmpdir(), "pi-safety-guardian-"));
  ...
  it("reports sandboxed exists permission failures instead of missing paths", async () => {
    const directory = await mkdtemp(join(tmpdir(), "pi-safety-guardian-permissions-"));
  ...
  it("bounds Guardian file reads before the helper writes stdout", async () => {
    const directory = await mkdtemp(join(tmpdir(), "pi-safety-guardian-read-bound-"));
  ...
  it("bounds Guardian directory entries inside the helper", async () => {
    const directory = await mkdtemp(join(tmpdir(), "pi-safety-guardian-entry-bound-"));
  ...
  it("protects the canonical global configuration path", () => {
    const paths = defaultProtectedWritePaths("/workspace/project", "/workspace/agent");
    expect(paths).toContain("/workspace/agent/safety.json");
  });
  ...
  it("refreshes Git metadata denies for later bash and file executions", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "pi-safety-git-refresh-worktree-"));
    const metadata = await mkdtemp(join(tmpdir(), "pi-safety-git-refresh-metadata-"));
  ...
  it("fails closed before executing when Git metadata becomes malformed", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "pi-safety-git-refresh-invalid-"));
  ...
  it("runs native file operations through the sandbox with one-call write roots", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "pi-safety-native-"));
