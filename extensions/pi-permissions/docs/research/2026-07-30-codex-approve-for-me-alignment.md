...
| `git init` | 只为语法安全、单段、当前目录的 `git init` 或 `git init .` 处理初始化；non-bare、空参数或其他目标一律不获 metadata grant。当前目录没有 `.git` 时，prospective root 仅是 `realpath(cwd)/.git`；不会借用父仓库 metadata，存在 malformed/unsafe 当前 metadata 时仍失败闭合。 |
